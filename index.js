    ...
    return new Promise(resolve => {
        const request = https.get(src, resp => {
            ...
            resp.on('end', () => {
                ...
                return mm.parseBuffer(buf, 'audio/mpeg').then(metadata => {
                    ...
                    if ((platform !== 'google' || metadata.format.bitrate < 24000 || metadata.format.bitrate > 96000) && metadata.format.bitrate != 48000) {
                        errors.push({ type: 'audio', value: src, detail: `Invalid bit rate ${metadata.format.bitrate}` });
                    }
                    if ((platform !== 'amazon' || metadata.format.duration > 240000) && metadata.format.duration > 120000) {
...
function removeAudioFiles(parent, index, badAudio, element) {
    ...
    if (element.name === 'audio' && element.attributes.src && badAudio.indexOf(element.attributes.src) > -1) {
        // Remove this please
...
module.exports = {
    check: function (ssml, options) {
        ...
        return ssmlCheckCore.check(ssml, options).then(coreErrors => {
            return coreErrors;
        });
    },
    verifyAndFix: function (ssml, options) {
        ...
        return ssmlCheckCore.verifyAndFix(ssml, options).then(result => {
            return result;
        });
    },
};
